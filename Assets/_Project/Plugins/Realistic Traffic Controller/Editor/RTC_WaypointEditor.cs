...
public class RTC_WaypointEditor : Editor {
    ...
    public static void OnDrawSceneGizmos(RTC_Waypoint waypoint, GizmoType gizmoType) {
        ...
        if (waypoint.previousWaypoint)
            Gizmos.DrawMesh((Mesh)Resources.Load("Arrow", typeof(Mesh)), waypoint.transform.InverseTransformPoint(Vector3.Lerp(waypoint.transform.position, waypoint.previousWaypoint.transform.position, .5f) + Vector3.up), Quaternion.identity, new Vector3(1f, 1f, 1f));
