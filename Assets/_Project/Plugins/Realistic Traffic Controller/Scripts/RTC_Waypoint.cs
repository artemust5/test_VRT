...
public class RTC_Waypoint : MonoBehaviour
{
    ...
    public void UpdateWaypoint()
    {
        ...
        if (interConnectionWaypoint)
            distanceToInterConnectionWaypoint =
                Vector3.Distance(transform.position, interConnectionWaypoint.transform.position);
        else
        ...
        if (nextWaypoint)
            angleToNextWaypoint =
                Vector3.Angle(nextWaypoint.transform.position - transform.position, transform.forward);
        else
            angleToNextWaypoint = 0f;

        //  Calculate angle to interconnection waypoint.
        if (interConnectionWaypoint)
            angleToInterConnectionWaypoint =
                Vector3.Angle(interConnectionWaypoint.transform.position - transform.position, transform.forward);
        else
        ...
        desiredSpeedForInterConnectionWaypoint =
            targetSpeed * (1f - Mathf.InverseLerp(0, 180, angleToInterConnectionWaypoint));
    }
