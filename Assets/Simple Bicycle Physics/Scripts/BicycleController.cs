...
namespace Simple_Bicycle_Physics.Scripts
{
    // Cycle Geometry Class - Holds Gameobjects pertaining to the specific bicycle
    [System.Serializable]
    public class CycleGeometry
    {
        public GameObject steerPivot, handles, lowerFork, fWheelVisual, rWheel;
        public GameObject crank, lPedal, rPedal, fGear, rGear;
    }
    ...
    public class BicycleController : MonoBehaviour
    {
        ...
        public static bool IsBraking { get; private set; }
        public float initialRbDrag = 0.5f;
        ...
        RaycastHit hit;
        [HideInInspector]
        public float customSteerAxis, customLeanAxis, customAccelerationAxis, customBrakeAxis, rawCustomAccelerationAxis;
        bool isRaw, sprint;
        [HideInInspector]
        ...
        Quaternion initialLowerForkLocalRotaion, initialHandlesRotation, initialSteerPivotRotation;
        ...
        void Start()
        {
            ...
            initialSteerPivotRotation = cycleGeometry.steerPivot.transform.localRotation;
        ...
        void FixedUpdate()
        {
            ...
            fPhysicsWheel.transform.rotation = Quaternion.Euler(transform.rotation.eulerAngles.x, 
                transform.rotation.eulerAngles.y + customSteerAxis * steerAngle.Evaluate(rb.velocity.magnitude) * (axisAngle/10f) + oscillationSteerEffect, 0);
            ...
            currentTopSpeed = !sprint ? Mathf.Lerp(currentTopSpeed, topSpeed * relaxedSpeed, Time.deltaTime) : Mathf.Lerp(currentTopSpeed, topSpeed, Time.deltaTime);

            if (velocityMagnitude < currentTopSpeed && rawCustomAccelerationAxis > 0)
                rWheelRb.AddTorque(transform.right * (torque * customAccelerationAxis));

            if (velocityMagnitude < currentTopSpeed && rawCustomAccelerationAxis > 0 && !isAirborne && !isBunnyHopping)
                rb.AddForce(transform.forward * accelerationCurve.Evaluate(customAccelerationAxis));

            if (velocityMagnitude < reversingSpeed && rawCustomAccelerationAxis < 0 && !isAirborne && !isBunnyHopping)
            ...
            if (stuntMode)
                rb.centerOfMass = boxCollider.center;
            else
                rb.centerOfMass = Vector3.zero + centerOfMassOffset;
            
            //SteerPivot
            cycleGeometry.steerPivot.transform.localRotation = initialSteerPivotRotation * Quaternion.Euler(0, customSteerAxis * steerAngle.Evaluate(velocityMagnitude) * (axisAngle/10f) + oscillationSteerEffect * 5, 0);
            var rearWheelAngle = rPhysicsWheel.transform.localRotation.eulerAngles.x;
            cycleGeometry.fWheelVisual.transform.GetChild(0).localRotation = 
                Quaternion.Euler(rearWheelAngle, 0, 0);
            
            
            //Handles
            //cycleGeometry.handles.transform.localRotation = Quaternion.Euler(0, customSteerAxis * steerAngle.Evaluate(velocityMagnitude) + oscillationSteerEffect * 5, 0) * initialHandlesRotation;

            //LowerFork
            //cycleGeometry.lowerFork.transform.localRotation = Quaternion.Euler(0, customSteerAxis * steerAngle.Evaluate(velocityMagnitude) + oscillationSteerEffect * 5, customSteerAxis * -axisAngle) * initialLowerForkLocalRotaion;
            ...
            //FWheelVisual
             ...
             // cycleGeometry.fWheelVisual.transform.rotation = Quaternion.Euler(xQuat * (customSteerAxis * -axisAngle), customSteerAxis * steerAngle.Evaluate(velocityMagnitude) + oscillationSteerEffect * 5, zQuat * (customSteerAxis * -axisAngle));
            ...
            {
                crankSpeed += Mathf.Sqrt(customAccelerationAxis/2f * Mathf.Abs(Mathf.DeltaAngle(crankCurrentQuat, crankLastQuat) * pedalAdjustments.pedalingSpeed));
                crankSpeed %= 360;
            }
            ...
            //CycleOscillation
            if ((sprint && velocityMagnitude > 5 && isReversing == false) || isAirborne || isBunnyHopping)
            ...
            cycleOscillation = -Mathf.Sin(Mathf.Deg2Rad * (crankSpeed + 90)) * (oscillationAmount * (Mathf.Clamp(currentTopSpeed / velocityMagnitude, 1f, 1.5f))) * pickUpSpeed;
            turnLeanAmount = -leanCurve.Evaluate(customLeanAxis) * Mathf.Clamp(velocityMagnitude * 0.1f, 0, 1);
            oscillationSteerEffect = cycleOscillation * Mathf.Clamp01(customAccelerationAxis) * (oscillationAffectSteerRatio * (Mathf.Clamp(topSpeed / velocityMagnitude, 1f, 1.5f)));
            ...
            {
                if (rb.drag < 500f)
                    rb.drag += customBrakeAxis * brakeForce * (10f/currentSpeed) * Time.unscaledDeltaTime;
            }
            else
            {
                rb.drag = initialRbDrag;
            }
        }
        void Update()
        {
        ...
        public void SetCustomSteerInput(float value, float sensitivity = 10f, float gravity = 5f)
        {
            var t = Time.unscaledDeltaTime;
            if (value != 0)
                customSteerAxis = Mathf.Clamp(Mathf.Lerp(customSteerAxis, value, sensitivity * t), -1f, 1f);
            else
                customSteerAxis = Mathf.Clamp01(Mathf.Abs(customSteerAxis) - gravity * t) * Mathf.Sign(customSteerAxis);
        }

        public void SetCustomAccelerationInput(float value, float sensitivity = 1f, float gravity = 5f)
        {
            var t = Time.unscaledDeltaTime;
            
            if (value != 0)
            {
                customAccelerationAxis = value * sensitivity * t * 40f;
            }
            else
                customAccelerationAxis = Mathf.Clamp01(Mathf.Abs(customAccelerationAxis) - gravity * t) * Mathf.Sign(customAccelerationAxis);

            rawCustomAccelerationAxis = value;
        }

        public void SetCustomBrakeInput(float value)
        {
            if (value > 0.1f)
            {
                IsBraking = true;
                customBrakeAxis = Mathf.Clamp01(value);
            }
            else
            {
                IsBraking = false;
        ...
        public void ActivateAdditionalKeyboardButtons()
        {
            sprint = Input.GetKey(KeyCode.LeftShift);
            wheelieInput = Input.GetKey(KeyCode.LeftControl);
        }

        public void SetSteeringOnOff(bool turnOn)
        {
            axisAngle = turnOn ? initialAxisAngle : 0f;
        }
